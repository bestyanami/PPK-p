...
parameterEvaluationUI <- function(id) {
  ns <- NS(id)
  tagList(
    box(title = "参数评估", status = "primary", solidHeader = TRUE,
        selectInput(ns("selected_result"), "选择模型结果",
                    choices = NULL),
        actionButton(ns("evaluate_params_button"), "评估参数")
    ),
    box(title = "参数估计值", status = "info", solidHeader = TRUE,
        DT::dataTableOutput(ns("params_table"))
    ),
    box(title = "参数分布图", status = "warning", solidHeader = TRUE,
