    ...
    box(title = "上传数据文件", status = "primary", solidHeader = TRUE,
        fileInput(ns("data_file"), "选择 CSV 文件",
                  ...
                  accept = c("text/csv",
                             ...
                             ".csv")),
        textInput(ns("col_id"), "ID 患者编号列名", value = "ID"),
        textInput(ns("col_time"), "TIME 给药或测量的时间点（小时）列名", value = "TIME"),
        textInput(ns("col_dv"), "DV 观测值列名", value = "DV"),
        textInput(ns("col_amt"), "AMT 给药剂量列名", value = "AMT"),
